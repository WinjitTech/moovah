...
try:
    ...
    c.execute("select * from Article where ModifiedDate > '" + (lastSyncDateTime) + "';")


    results = c.fetchall()

    for result in results:
        strQuery = "select * from ArticleBoxMapping where ReferenceID = " + str(result['Id'])
        c.execute(strQuery)
        results2 =c.fetchall()
