...
try:
    ...
    c.execute("select * from Retailer where ModifiedDate > '" + (lastSyncDateTime) + "';")


    results = c.fetchall()

    for result in results:
        strQuery = "select * from RetailerBranch where RetailerID = " + str(result['Id'])
        c.execute(strQuery)
        results2 =c.fetchall()
        result['BranchDetails'] = results2
