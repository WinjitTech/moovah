...
def DownloadFileToPath(RemotePath, LocalPath, SectionEnumVal, RecordID):
    try:
        ...
        if status == "200":
            ...
            FillReports.InsertIntoboxsyncdetailslog(RemotePath, 0, 'CMS Error Code: ' + str(status), RecordID, 0,
                                                    SectionEnumVal)
        ...
        FillReports.InsertIntoboxsyncdetailslog(RemotePath, 0, 'Internet connection not avialable', RecordID,
                                                SectionEnumVal)
 ...
 #   print "RemoteURl: "+str(RemoteURL)
    try:
        ...
        if key in ContentTokens and RemoteURL is not None and RemoteURL is not '':
            parse_object = urlparse.urlparse(RemoteURL)
            LocaPath += parse_object.path
            print "localpath:"+str(LocaPath)
            RemoteURL = str(RemoteURL).replace(".net", ".com")

            if ".mp4" in RemoteURL:

                if not DownloadFileToPath(str(RemoteURL[:-3] + "3gp"), str(LocaPath[:-3] + "3gp"), SectionEnumVal,
                                          RecordID):
                    print("File Downloaded Failed for: " + str(LocaPath[:-3] + "3gp"))
