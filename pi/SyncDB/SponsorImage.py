...
def GetSponsorImages():
	...
	print url
		...
		i=0
		    ...
		    #print SponsorImage
	            strQuery="insert into generalSettings(Id,Name,Instructions,IsEarnMoreSponsor,SponsorURL,SponsorImage,IsActive) values('"+str(js['ReturnObject'][i]['Id'])+"','"+str(js['ReturnObject'][i]['Name'])+"','"+str(js['ReturnObject'][i]['Instructions'])+"','"+str(js['ReturnObject'][i]['IsEarnMoreSponsor'])+"','"+str(js['ReturnObject'][i]['SponsorURL'])+"','"+str(js['ReturnObject'][i]["SponsorImage"])+"',1)"
		    c.execute(strQuery);
                    i=i+1
