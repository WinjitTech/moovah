            ...
            for key in jsondata[i].keys():
                ...
                if "IsActive" in jsondata[i].keys():
                    if jsondata[i]["IsActive"]==1:
                        FileMgr.CheckAndDownloadFile(key,value,7,Id)
                else:
                    FileMgr.CheckAndDownloadFile(key,value,7,Id)
...
imgpath = GetCMSData.GetData('Box')

insertOrUpdateRecord(imgpath,"Box")

MoovahLogger.logger.info("Box records updated successfully")