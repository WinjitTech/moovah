...
def insertOrUpdateRecord(jsondata,tblName,ParentID):
    ...
    if 'ArticleBoxMapping' in tblName and ParentID is not None:
            ...
            for key in jsondata[i].keys():
                ...
                if "IsActive" in jsondata[i].keys():
                    if jsondata[i]["IsActive"]==1:
                        FileMgr.CheckAndDownloadFile(key,value,16,Id)
                else:
                    FileMgr.CheckAndDownloadFile(key,value,16,Id)
                ...
                if 'BoxMappings' in key:
                        insertOrUpdateRecord(value,"ArticleBoxMapping",Id)
                else:
...
imgpath = GetCMSData.GetData('Articles')

insertOrUpdateRecord(imgpath,"Article",None)

MoovahLogger.logger.info("Articles updated successfully")