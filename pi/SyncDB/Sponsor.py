            ...
            for key in jsondata[i].keys():
                ...
                if "IsActive" in jsondata[i].keys():
                    if jsondata[i]["IsActive"]==1:
                        FileMgr.CheckAndDownloadFile(key,value,19,Id)
                else:
                    FileMgr.CheckAndDownloadFile(key,value,19,Id)
                ...
                if 'SponsorBoxDetails' in key:
                    insertOrUpdateRecord(value,'SponsorBoxMapping')
                else:
...
imgpath = GetCMSData.GetData('Sponsors')

insertOrUpdateRecord(imgpath,"sponsor")

MoovahLogger.logger.info("Sponsors updated successfully")