            ...
            for key in jsondata[i].keys():
                ...
                if "IsActive" in jsondata[i].keys():
                    if jsondata[i]["IsActive"]==1:
                        FileMgr.CheckAndDownloadFile(key,value,13,Id)
                else:
                    FileMgr.CheckAndDownloadFile(key,value,13,Id)
                ...
                if 'BoxMappings' in key:
                    insertOrUpdateRecord(value,'BooksBoxMapping')
                else:
                    if 'BoxMappings' in key:
...
imgpath = GetCMSData.GetData('Books')

insertOrUpdateRecord(imgpath,"books")

MoovahLogger.logger.info("books updated successfully")