            ...
            for key in jsondata[i].keys():
                ...
                if "IsActive" in jsondata[i].keys():
                    if jsondata[i]["IsActive"]==1:
                        FileMgr.CheckAndDownloadFile(key,value,15,Id)
                else:
                    FileMgr.CheckAndDownloadFile(key,value,15,Id)
                ...
                if 'AlbumDetails' in key:
                    ...
                    if 'BoxMappings' in key:
                        insertOrUpdateRecord(value,"RSSBoxMapping")
                    else:
...
imgpath = GetCMSData.GetData('NewsFeeds')

insertOrUpdateRecord(imgpath,"rss")

MoovahLogger.logger.info("NewsFeed updated successfully")