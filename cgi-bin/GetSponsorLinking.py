...
try:
    ...
    c.execute("select * from SponsorLinking where ModifiedDate >= '" + (lastSyncDateTime) + "';")


    results = c.fetchall()

    for result in results:
        strQuery = "select * from SponsorLinking where ModuleID = " + str(result['Id'])
        c.execute(strQuery)
        results2 =c.fetchall()
        #result['SponsorLinking'] = results2
