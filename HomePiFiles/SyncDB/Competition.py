        ...
        try:
            for key in jsondata[i].keys():
                ...
                if 'CompetitionMapping' in key:
                    insertOrUpdateRecord(value,'CompetitionBoxMapping')
                else:
                    if 'CompetitionQuestions' in key and not 'CompetitionQuestionsOptions' in key:
                        insertOrUpdateRecord(value,'CompetitionQuestions')
                    else:
                        if 'CompetitionQuestionsOptions' in key:
                            insertOrUpdateRecord(value,"CompetitionQuestionOptions")
                        else:
                            if 'CompetitionWinners' in key:
                                insertOrUpdateRecord(value,"CompetitionWinner")
                            else:
...
insertOrUpdateRecord(imgpath,"Competition")
