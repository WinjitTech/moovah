            ...
            for key in jsondata[i].keys():
                ...
                if "IsActive" in jsondata[i].keys():
                    if jsondata[i]["IsActive"]==1:
                        FileMgr.CheckAndDownloadFile(key,value,8,Id)
                else:
                    FileMgr.CheckAndDownloadFile(key,value,8,Id)
                ...
                if 'BranchDetails' in key:
                    insertOrUpdateRecord(value,'RetailerBranch')
                else:
...
imgpath = GetCMSData.GetData('Retailers')

insertOrUpdateRecord(imgpath,"Retailer")

MoovahLogger.logger.info("Retailers updated successfully")