...
def insertOrUpdateRecord(jsondata,tblName):
    ...
    for i in range(len(jsondata)):
        ...
        if "BoxID" in jsondata[i].keys() and "DealID" in jsondata[i].keys():
            query ="delete from "+tblName+" where BoxID='"+str(jsondata[i]['BoxID'])+"'" + " and DealID='"+str(jsondata[i]['DealID'])+"'"
            print('Record deleted for '+tblName+' :'+str(Id))
            c.execute(query)

        if "RetailerBranchID" in jsondata[i].keys() and "DealID" in jsondata[i].keys():
            query ="delete from "+tblName+" where RetailerBranchID='"+str(jsondata[i]['RetailerBranchID'])+"'" + " and DealID='"+str(jsondata[i]['DealID'])+"'"
            print('Record deleted for '+tblName+' :'+str(Id))
            c.execute(query)
        ...
        try:
            for key in jsondata[i].keys():
                ...
                if 'DealLocations' in key:
                    insertOrUpdateRecord(value,'DealLocations')
                else:
                    if 'DealMappings' in key:
                        insertOrUpdateRecord(value,"DealsBoxMapping")
                    else:
...
insertOrUpdateRecord(imgpath,"Deals")
