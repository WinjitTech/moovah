            ...
            for key in jsondata[i].keys():
                ...
                if "IsActive" in jsondata[i].keys():
                    if jsondata[i]["IsActive"]==1:
                        FileMgr.CheckAndDownloadFile(key,value,17,Id)
                else:
                    FileMgr.CheckAndDownloadFile(key,value,17,Id)
...
imgpath = GetCMSData.GetData('BannerImages')

insertOrUpdateRecord(imgpath,"BannerImages")

MoovahLogger.logger.info("Banner images updated successfully")